...
namespace glge::proc_gen
{
	...
	using step_pts = std::tuple<std::optional<float>,
								std::optional<float>,
								std::optional<float>,
								std::optional<float>>;
	...
	grid_get_safe(const Matrix<float> & grid, size_t x, size_t y)
	{
		if (x >= grid.width())
		{
			return std::nullopt;
		...
		return std::make_optional(grid[grid.index(
	...
	average(const std::tuple<Args...> vals) noexcept
	{
		...
		return std::apply([&](auto... v) { return sum(extract(v)...); }, vals) /
			   ct;
	}
	...
	step_pts
	get_diamond(const Matrix<float> & grid, size_t radius, grid_pt center)
	{
		auto [x, y] = center;
	...
	void diamond_step(const RNG & rng, Matrix<float> & grid, size_t radius)
	{
		...
		{
			for (size_t y = offset ? radius : 0; y < grid.height();
				 y += (2 * radius))
			{
	...
	step_pts
	get_square(const Matrix<float> & grid, size_t radius, grid_pt center)
	{
		auto [x, y] = center;
	...
	void
	diamond_square(std::uint32_t seed, Matrix<float> & grid, float variation)
	{
		...
		if (variation < 0.0f)
		{
			throw std::invalid_argument(
				EXC_MSG("Variation cannot be negative"));
		}
