...
namespace glge::renderer::primitive
{
	namespace opengl
	{
		...
		public:
			GLLines(const vector<vec3> & points) : vertex_count(util::safe_cast<size_t, GLsizei>(points.size())),
				destroy(false)
			{
				glGenVertexArrays(static_cast<GLsizei>(VAO.size()), VAO.data());
				glGenBuffers(static_cast<GLsizei>(VBO.size()), VBO.data());

				renderer::opengl::throw_if_gl_error(EXC_MSG("Failed to generate requisite storage for lines"));
...

				{
					...
					bind_attrib_data(VBO[vertex_index], vertex_index, points, false);
				}
			}

			GLLines(const GLLines &) = delete;

			GLLines(GLLines && other) : vertex_count(other.vertex_count), VAO(other.VAO), VBO(other.VBO), 
			...
			~GLLines()
			{
				if (destroy)
				{
					glDeleteVertexArrays(util::safe_cast<typename decltype(VAO)::size_type, GLsizei>(VAO.size()), VAO.data());
					glDeleteBuffers(util::safe_cast<typename decltype(VBO)::size_type, GLsizei>(VBO.size()), VBO.data());
				}
			}
