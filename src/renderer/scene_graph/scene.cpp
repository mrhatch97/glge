...
{
	NodeHandle::NodeHandle(observer_ptr<Node> parent,
						   Node & node,
						   Scene & scene) :
		parent(parent),
	...
	CameraHandle::CameraHandle(observer_ptr<Node> parent,
							   SceneCamera & node,
							   Scene & scene) :
	...
	Scene::Scene() :
		root(std::make_unique<Node>()), settings(nullptr, false, false)
	{}
